...
function newTweet(req,res){
	...
	if(req.session.data){
		...
		var data = {"email": req.session.data.email, "tweet":req.body.newTweet,"tweetId":tweetId ,"twitterHandle":req.session.data.twitterHandle,"dateTime":today,"name":req.session.data.name,"methodName":"newTweet"};
		mq_client.make_request('home_queue',data, function(err,results){
			console.log(results);
			if(err){
				throw err;
			}
...
function reTweet(req,res){
	...
	if(req.session.data){
		...
		var data = {"tweetId_Request": req.body.retweetId, "email": req.session.data.email, "tweet":req.body.newTweet,"tweetId":tweetId ,"twitterHandle":req.session.data.twitterHandle,"dateTime":today,"name":req.session.data.name,"methodName":"reTweet"};
		mq_client.make_request('home_queue',data, function(err,results){
			console.log(results);
			if(err){
				throw err;
...
function homeData(req,res){
	...
	if(req.session.data){
		...
		var data = {"email": req.session.data.email,"methodName":"homeData"};
		mq_client.make_request('home_queue',data, function(err,results){
			console.log(results);
			if(err){
				throw err;
...
//					coll2.find({"email": req.session.data.email}).sort({"dateTime":1}).toArray(function(err, tweets) {
//						 if(err){
...
function searchTweet(req,res){
	...
	if(req.session.data){
		...
		var data = {"hashTag": (req.body.hashTag),"email":req.session.data.email,"methodName":"searchTweet"};
		mq_client.make_request('home_queue',data, function(err,results){
			console.log(results);
			if(err){
				throw err;
